 ...
 const emailForgottenPassword = async (datos) => {
    const transport = nodemailer.createTransport({
        host: process.env.EMAIL_HOST,
        port: process.env.EMAIL_PORT,
        auth: {
    ...
    await transport.sendMail({
        from: 'BienesRaices.com',
        to: email,
        subject: 'Reestablece tu cuenta en BienesRaices',
        text: 'Da click en el enlace para confirmar tu cuenta',
        html: `
            <p>Hola ${name}, has solicitado restablecer tu password de Bienes Raices.com</p>
